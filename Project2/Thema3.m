...
%Η ελαχιστοποίηση θέλει τις λιγότερες επαναλήψεις(5) και μετά ο armijo(6).
%Τις περισσότερες επαναλήψεις χρειάζεται η μέθοδος σταθερού βήματος γ(79).
...
%Σημείο iii --> (1,-1). Για armijo(2reps) και σταθερό γ(171reps)
%καταλήγει στο 0, το οποίο είναι τοπικό ελάχιστο. Δεν βρίσκει το ολικό 
%ελάχιστο γιατί το grad της f κοντά στο 0,
%προσεγγίζει το 0, οπότε πληρείται η συνθήκη τερματισμού του αλγορίθμου.
%Με την ελαχιστοποίηση όμως βρίσκουμε το πραγματικό ελάχιστο της συνάρτησης
%σε μόλις 5 επαναλήψεις.
[min_f_A, min_x_A, min_y_A, f_values_A] = Newton(f, 1, -1, epsilon, gamma);
[min_f_B, min_x_B, min_y_B, f_values_B] = Newton(f, 1, -1, epsilon);
[min_f_C, min_x_C, min_y_C, f_values_C] = Newton(f, 1, -1, epsilon, 'armijo', true);
figure(2);
